...
void initialise_lattice_CZTS_randomized()
{
    ...
    int swaps=X*Y*Z*100000; // Defining number of swap attempts based on tota no. of sites
    int i;
    int x_swap1, y_swap1, z_swap1, x_swap2, y_swap2, z_swap2, tmp_species;
    for (i=0;i<swaps;i++)
    {
      // Choosing two random sites to swap
      x_swap1 = rand_int(X)+1;
      y_swap1 = rand_int(Y)+1;
      z_swap1 = rand_int(Z)+1;
      x_swap2 = rand_int(X)+1;
      y_swap2 = rand_int(Y)+1;
      z_swap2 = rand_int(Z)+1;
